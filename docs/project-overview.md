...
- **v1.4.7 收口**:Rail 分区折叠改引用 `collapsedSections`、折叠态页脚适配 52px 轨;HomeView busy 判定与 ChatView 对齐;TitleBar 明暗图标按 `themes.js` group 判定;DiffView 恢复 + Ctrl+N;**设置页全窗化**(不渲染 Rail,顶部返回/关闭按钮,返回原视图)+ **窄窗口 Rail 不再消失**(900px 以下强制 52px 图标轨);renderer build / Electron 冒烟加固;截图复核 8 视图全绿(修复 smoke 设置选择器、窄屏 rail-off 空列、改动时间格式化);全量回归 1012 单测 + Electron 冒烟通过。
...
- **v1.8.4 前端布局工效重构与遥测体系落地**:① Rail 侧边栏折叠重构——图标容器居左 10px 固定、中心 X 坐标恒定 28px(收起/展开绝对同轴零位移)，收起时 80ms 文字先淡出、240ms 平滑宽度过渡彻底消除布局抖动，顶部 36×36px 独立新建会话钮，右浮动工具提示，底部操作垂直单列排布规避 30px 溢出；② 遥测数据流落地——消息气泡底部对齐图 1 遥测栏(复制/点赞/点踩/分享/数据库图标 + Token 用量 + 时间戳)，输入框底部署对齐图 2 遥测条(轮次与步数 · 速率 + 累计 Token 与缓存命中率 + 上下文占用环形进度)；③ 原创高工效调色板——升级 5 套原创主题语义色与强调色(Inkstone Cyan, Obsidian Iris, Sunset Amber, Aurora Spruce, Steel Teal)，严守 10 主题结构槽与暗/明基座一致性，WCAG 对比度 110/110 PASS，TUI 调色板逐字节同步；④ 右侧 Dock 新增 Plan 专属面板——集成活动卡片与步骤编排树，支持 Ctrl+J 快捷调度。**内核零改动**。
- **v1.8.3 前端审查修复**:逐条修复 v1.8.1–v1.8.2 的 code review 问题清单。① TUI 调色板生成物与 `tokens.css` 失同步——生成逻辑抽为纯函数 `buildPalette`/`buildPaletteBody`,新增**逐字节一致**守卫(`paper --warn` 214→136);② 闸门 `EXTRA_GATE` 由死代码改为经 `checkPairs` 实检,并加「`GATE` 恒 11 对」锁定用例;③ 迁回 v1.8.2 漏掉的**修饰符级**规则(`.cz-meta.compact` 含窄窗隐藏规则 → **compact 偏好恢复生效**、`.api-item .ai-ic`、`.f-in.short`/`.mid`),并以**自动守卫** `shell-class-migration.test.js`(从 JSX 反推 + 须给理由的 ALLOW)取代人工清单;④ 修 `ChatView` 的 `css.mode` 未定义致 className 渲染出字面量 `undefined`,新增 `css-module-members.test.js`;⑤ 死状态清理:`railMode`/`railView`/`contextCollapsed` 及三个对应 action 随三列壳层退役,旧偏好文件残留键被归一丢弃;⑥ `no-dsh-imports` 守卫扩展到 `main.js`/`preload.js`/`kernel-host.js`;symlink 用例加平台能力门控(断言未削弱)。**内核零改动**。
- **v1.8.1 壳层重构(布局 + token C 变体)**:三列可拖拽 grid(AppFrame/columns.js)+ 40px 原生标题栏 overlay + 右栏 dock(文件树/改动/恢复)+ 设置改模态(`settingsOpen`/`settings_toggled`,`Ctrl+,`);结构槽明/暗两档中性阶梯(`#ffffff`/`#151517` 等),10 主题只贡献 accent,闸门 54 项;偏好键 `sidebarWidth` `rightbarWidth` `rightbarOpen` `dockTab` `chatTab`;删除自绘 TitleBar 与 shell.css;零引用外部参照代码。**内核零改动**。
- **v1.8.0 工学换肤(视觉语言层)**:10 主题重编为**四段带**(暗 3 / 柔暗 2 / 柔明 2 / 明 3——保存 `sumi`/`nord`/`latte`/`paper` 原值,新增 `slate`/`vesper`/`ash`/`snow`/`sand`/`lotus`,退役 6 套静默迁移);对比度闸门(11 对 × 10 套 = 110 项)升级为**常驻单测** `scripts/check-theme-contrast.mjs`(`110/110 PASS`),亮度实现单一来源 `gui/src/state/contrast.js`;新增主题中枢 `gui/src/state/theme-hub.js`(`nextInGroup`/`otherModeTheme`/`groupOf`,同组轮换 + 跨模式切换 + 退役 id 归一)与设置›外观主题网格(四段分组 + 运行时实测色板 + 玻璃开关);标题栏/composer/浮层去硬编码色(全走 tokens / `color-mix` 与 `theme.css` `[theme]` 别名块的派生变量 `--accent-glow`/`--bg-glass`/`--ok-bg`/`--warn-bg`/`--err-bg`/`--info-bg`/`--bg-raised`);卡片两段式(`.ev-h` 由 `div` 改 `<button type="button">`)+ 工具耗时(`tool-result` 描述符新增 `durationMs`)+ 推理摘要卡(`model:response` → quiet 的 `thought`,门控出卡);新增偏好 `lastDark`/`lastLight`/`glass`;新增 `theme-pins.test.js`/`dom-contract.test.js`/`theme-contrast.test.js`/`theme-hub.test.js` 冻结契约。**内核零改动**(八目录 diff 为空)。
