...
当前版本 **v1.8.4**(2026-09-23,前端补丁:前端布局工效重构与遥测体系落地 —— Rail 零抖动平滑开合 + 图 1 图 2 遥测体系 + 5 套原创调色板与对比度闸门自洽 + Dock 计划面板集成)。此前:v1.8.3 前端审查修复,v1.8.2 前端缺陷修复,v1.8.1 壳层重构(三列可拖拽 + 右栏 dock + 设置模态 + 中性阶梯 token),v1.8.0 工学换肤。首个正式版本 v1.0.0 整合此前全部内部迭代;完整能力总结与后续版本日志见 [`CHANGELOG.md`](CHANGELOG.md),版本升级判定见上文[版本命名规则](#版本命名规则)。**每个已发布版本在 git 中有对应的带注解 tag**(`git tag -l` / `git show v1.7.2`)。
