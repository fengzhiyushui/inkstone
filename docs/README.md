...
| minor | `v1.9.0` → `v1.10.0` | 大版本规划内的小功能：slash 命令、内置工具、配置档、一门语言的语义支持、一次前端对齐 | 规划内可直接推进 |
| patch | `v1.10.0` → `v1.10.1` | 文档更新、小漏洞修复、测试补齐、依赖小升级；不改功能面 | 随手推进 |
...
在项目日常开发、功能迭代与缺陷修复过程中，必须严格遵守以下开发分支与提交规则：

1. **新版本独立分支开发**：
   - 针对新版本的开发，**必须从最新 `main` 主干迁出独立的本地开发分支**（分支命名推荐如 `feat/v1.11.0` 或 `fix/v1.10.1`）。
   - 严禁直接在 `main` 主干上直接开发提交，严禁在未清理隔离的旧版本分支上混杂开发。
   - 开发与质量测试闸门全部通过后，合入 `main` 并推送到远程，打上对应的注解 Git Tag，随后清理已完成的本地与远程分支。

2. **分支内严格语义化版本提交**：
   - 分支内的提交信息必须符合语义化版本更新要求，格式为标准的 `v<major>.<minor>.<patch>`。
   - **严禁使用 `vx.xx.xx-mx` 类似提交**：严禁在提交信息、Commit Scope 或版本声明中使用 `-m1`、`-m2`、`-m4` 等内部里程碑/阶段代号（例如：**严厉禁止**出现 `feat(v1.9.0-m1)`、`feat(v1.9.0-m4)` 等提交）。
   - **严格按照 `vx.xx.xx` 更新演进**：所有版本推进必须严格按照标准的语义化版本号更新（例如：`v1.11.1`、`v1.11.2` 等）。若有阶段性改动需要独立发布或阶段验证，应当按语义化规范递增 patch/minor 版本，确保每一次版本记录与提交均具备清晰、规范、无污染的标准语义版本号。
